...
class ggm(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.add_edge1 = nn.Linear(
            self.dim_of_graph_vector + dim_of_node_vector + N_conditions,
            dim_of_FC)
        self.add_edge2 = nn.Linear(dim_of_FC, dim_of_FC)
        self.add_edge3 = nn.Linear(dim_of_FC, N_bond_features)

        self.select_node1 = nn.Linear(
            dim_of_node_vector * 2 + dim_of_node_vector + N_conditions,
            dim_of_FC)
        self.select_node2 = nn.Linear(dim_of_FC, dim_of_FC)
        self.select_node3 = nn.Linear(dim_of_FC, 1)

        self.select_isomer1 = nn.Linear(
            dim_of_node_vector * 1 + dim_of_node_vector + N_conditions,
            dim_of_FC)
        ...
        self.cal_graph_vector1 = nn.Linear(dim_of_node_vector,
                                           self.dim_of_graph_vector)
        self.cal_graph_vector2 = nn.Linear(dim_of_node_vector,
                                           self.dim_of_graph_vector)
        self.cal_encoded_vector1 = nn.Linear(dim_of_node_vector,
                                             dim_of_node_vector)
        self.cal_encoded_vector2 = nn.Linear(dim_of_node_vector,
                                             dim_of_node_vector)

        self.init_graph_state1 = nn.Linear(self.dim_of_graph_vector,
                                           self.dim_of_graph_vector)  # not used
        self.init_node_state1 = nn.Linear(
            dim_of_node_vector + self.dim_of_graph_vector, dim_of_node_vector)
        self.init_edge_state1 = nn.Linear(
            self.dim_of_graph_vector + dim_of_edge_vector, dim_of_edge_vector)

        self.original_node_embedding = nn.Linear(
            N_atom_features + N_extra_atom_features, dim_of_node_vector,
            bias=False)
        self.original_edge_embedding = nn.Linear(
            N_bond_features + N_extra_bond_features, dim_of_edge_vector,
            bias=False)
        self.node_embedding = nn.Linear(N_atom_features, dim_of_node_vector,
                                        bias=False)
        self.edge_embedding = nn.Linear(N_bond_features, dim_of_edge_vector,
                                        bias=False)
    ...
    def forward(self, s1, s2, condition1, condition2, shuffle=False):
        ...
        g, h, scaffold_g, scaffold_h = utils.make_graphs(s1, s2,
        ...
        if condition.shape:
            latent_vector = torch.cat([latent_vector, condition.view(1, -1)],
                                      -1)
        ...
        for idx in leaves:
            ...
            for edge in edge_list:
                ...
                target = utils.create_var(utils.one_hot(
            ...
            end_add_edge = utils.create_var(
                utils.one_hot(torch.FloatTensor([N_bond_features - 1]),
                              N_bond_features))
            add_edge_losses.append(
        ...
        new_node = self.add_node(scaffold_g, scaffold_h, latent_vector)
        # Force the termination vector to be [0, 0, ..., 0, 1].
        end_add_node = utils.create_var(
            utils.one_hot(torch.FloatTensor([N_atom_features - 1]),
                          N_atom_features))
        add_node_losses.append(
    ...
    def sample(self, s1=None, s2=None, latent_vector=None, condition1=None,
               condition2=None, stochastic=False):
        """\
        Parameters
        ----------
        s1: whole SMILES str
        ...
        for null_index1 in range(max_add_nodes):
            ...
            if np.argmax(
                    new_node.data.cpu().numpy().ravel()) == N_atom_features - 1:
                break
            ...
            for null_index2 in range(max_add_edges):
                ...
                # Recall our definition of the termination vector:
                if np.argmax(
                        new_edge.data.cpu().numpy().ravel()) == N_bond_features - 1:
                    break
                ...
                selected_node = list(scaffold_h.keys())[np.argmax(
                    utils.probability_to_one_hot(selected_node,
                                                 stochastic).data.cpu().numpy().ravel())]
                if idx not in scaffold_g_save:
                    scaffold_g_save[idx] = []
                    scaffold_g[idx] = []
                scaffold_g_save[idx].append((new_edge, selected_node))
                scaffold_g[idx].append(
                    (self.init_edge_state(scaffold_h, new_edge), selected_node))
    ...
    def optimize(self, s1, s2, stochastic=False, lr=0.01, max_iter=100,
                 beta1=0.01):
        g, h, scaffold_g, scaffold_h = utils.make_graphs(s1, s2,
                                                         extra_atom_feature=True,
                                                         extra_bond_feature=True)
        ...
        for iteration in range(max_iter):
            ...
            prop = self.predict_property(
                torch.cat([latent_vector, scaffold_state], 1)).view(-1)
        ...
        for v in visited:
            ...
            new_prop = self.predict_property(
                torch.cat([latent_vector, scaffold_state],
                          1)).squeeze().data.cpu().numpy()
            loss1 = -0.5 * torch.sum(
                1 + logvar - mu.pow(2) - logvar.exp()).data.cpu().numpy()[0]
            ...
            selected_isomer, target = self.select_isomer(s1, latent_vector)
            new_s = isomers[
                np.argmax(selected_isomer.squeeze().data.cpu().numpy())]
            retval.append((new_s, objective, new_prop[0], loss1,
                           latent_vector.data.cpu().numpy()[0]))
        return retval
    ...
    def mpnn(self, g, h, U, C, condition_vector=None):
        ...
        if condition_vector is None or not condition_vector.shape:
            ...
            ls = torch.cat(
                [condition_vector for i in range(list(node_list1.size())[0])],
                0)
            messages = F.relu(
    ...
    def select_node(self, g, h, latent_vector):
        ...
        # collect node state
        vs = utils.collect_node_state(h,
                                      except_last=True)  # (len(h)-1, dim_of_node_vector)
        size = vs.size()
    ...
    def select_isomer(self, mother, latent_vector):
        ...
        for s in isomers:
            g, h = utils.make_graph(s, extra_atom_feature=True,
                                    extra_bond_feature=True)
            self.embede_graph(g, h)
    ...
    def init_node_state(self, h, atom_feature):
        ...
        return self.init_node_state1(
            torch.cat([graph_vector, self.node_embedding(atom_feature)], -1))

    def init_edge_state(self, h, edge_feature):
        """Return an initialized edge-state vector of shape (1, dim_of_edge_vector).
        See Eq. (11) of Yujia Li et al. 2018."""
        graph_vector = self.cal_graph_vector(h)
        return self.init_edge_state1(
            torch.cat([graph_vector, self.edge_embedding(edge_feature)], -1))
    ...
    def init_scaffold_state(self, scaffold_g, scaffold_h, condition):
        ...
        for k in range(len(self.init_scaffold_U)):
            self.mpnn(scaffold_g, scaffold_h, self.init_scaffold_U[k],
                      self.init_scaffold_C[k], condition)
